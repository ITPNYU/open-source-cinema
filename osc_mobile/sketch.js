  ...
  $.ajax( { url: myURL,
    ...
    success: function (data){  //create the select ui element based on what came back from db
      ...
      for (var i = 0; i < data.length; i ++ ){
        var thisItem = data[i];
        if (thisItem.type != "text") continue;
        ...
        //Find the element in your thing that is closest to this thing
        ...
        allOthers[otherPerson].ranking += smallestDistance; //add the distance to their total
    ...
    var smallestAverage = 100000;
    for(var name in allOthers){
      allOthers[name].ranking = allOthers[name].ranking/allOthers[name].elements.length;
      if(allOthers[name].ranking > biggestAverage) biggestAverage = allOthers[name].ranking;
      if(allOthers[name].ranking < smallestAverage) smallestAverage = allOthers[name].ranking;
    }
    //NOW VISUALIZE OTHERS ACCORDING TO HOW CLOSE THEIR TEXT IS
    clear();
    var pos = 0;

    var degreeIncrements = 2*PI/numberOfOthers;
    for(var name in allOthers){
      fill(random(255),random(255),random(255));
      //map font size from smallest to biggest from range found in other people
      var fontSize = map(allOthers[name].ranking, biggestAverage,smallestAverage,72,11);
      textSize(fontSize);
      var x = width/2 + 100*cos(degreeIncrements*pos);
      var y = height/2 + 100*sin(degreeIncrements*pos);
      text(name, x ,y);
      allOthers[name].centerX = x;
      allOthers[name].centerY = y;
      ...
      for(var i = 0; i < elements.length; i++){
         ...
         rotateZ(degreeIncrements*pos);
         text(elements[i].msg,0,0);
         pop();
      }*/
      pos += 1;
      //console.log(name);
